...
| QR/바코드 체크인 | 현장 입력이 가장 빠름 | 예약 알림/QR 발급/스캔 장치 또는 카메라 UX가 필요함 | 후속 고도화 |
| 키오스크 셀프 체크인 | 직원 업무량 감소 | 별도 화면/장비/현장 운영 정책 필요 | 후속 고도화 |
| 휴대폰 인증번호 확인 | 오접수 방지에 강함 | 외부 인증/문자 발송 연동이 필요함 | MVP 범위 밖 |
...
| 발견 시점 | 추가 작업 | 이유 | 처리 |
|---|---|---|---|
| 접수 UX 검토 | QR/바코드 체크인 | 예약번호 검색보다 더 빠른 현장 접수 가능 | 후속 고도화 |
