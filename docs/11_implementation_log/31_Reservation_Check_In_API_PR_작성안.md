...
| 케이스 | 확인 방법 | 기대 결과 |
|---|---|---|
| 예약 상세 상태 확인 | `GET /api/reservations/{id}` | `data.status = CHECKED_IN` |
| 체크인 이후 예약 취소 | `DELETE /api/reservations/{id}` | HTTP 409, `error.code = RESERVATION_CANCEL_INVALID_STATUS` |
