...
- 기본 대기열 화면은 진행 중 항목만 표시
- 완료/취소/미응답/전체 내역을 볼 수 있는 조회 탭 추가
...
- [x] Edge: 기본 진행 중 탭에서 완료/취소/미응답 항목 숨김
- [x] Edge: 완료/취소/미응답/전체 탭에서 종료 내역 조회
...
- 전체 탭은 백엔드가 전체 상태 조회 파라미터를 별도로 제공하지 않아 상태별 조회를 병합합니다.
