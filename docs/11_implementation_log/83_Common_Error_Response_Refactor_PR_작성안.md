...
| GitNexus | CLI status 결과 stale. MCP impact 도구 미노출로 `rg` 기반 영향 범위 확인 |
...
- GitNexus MCP impact 도구가 현재 세션에 없고 CLI index가 stale 상태입니다. `detect_changes`는 repo 지정 오류 후 `--repo health-center-smart-reservation`로 재시도했지만 출력 없이 exit code 1로 실패했습니다. 대신 `rg`, Maven, Next build로 보완했습니다.
