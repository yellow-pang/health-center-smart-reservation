...
- 포함:
  ...
  - [x] 초기 인증 확인 중 로그인 폼 대신 로딩 화면 표시
...
- 초기 인증 확인 중에는 이메일/비밀번호 폼을 바로 노출하지 않고 `로그인 상태 확인 중` 로딩 화면을 보여준다.
...
8. `/login` 진입 직후 초기 인증 확인 중에는 로그인 폼 대신 로딩 화면이 잠깐 표시되는지 확인
...
| 수정 요청 | 로그인 화면 재진입 시 일시적으로 폼이 보이지 않도록 초기화/체크 중 로딩 화면 표시 | 반영 |
