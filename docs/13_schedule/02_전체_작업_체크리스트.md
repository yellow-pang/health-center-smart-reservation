...
2026-05-17 현재 상태 재확인 결과, 작업 트리는 점검 전 기준으로 깨끗했고 루트 `docker-compose.yml`, `backend/Dockerfile`, `frontend/Dockerfile`, `infra/jenkins/Dockerfile`, `infra/jenkins/docker-compose.jenkins.yml`, `Jenkinsfile`, `.env.example`이 존재함을 확인했다. `git diff --check`, `mvn.cmd -q -DskipTests compile`, `mvn.cmd -q test-compile`, `npm.cmd run build`는 통과했다. GitNexus는 `.git`이 있는 repository 루트에서 실행해야 하며, `backend`/`frontend` 하위 폴더에서는 repository를 찾지 못할 수 있다. CLI 또는 MCP가 실패하면 변경 범위 확인은 `git status`, `git diff`, `rg`, Maven/Next build로 보완한다.
...
- [x] GitNexus와 `rg` 사용 기준 확인
  ...
  - 검증 방법: repository 루트에서 `gitnexus status`, 필요 시 `gitnexus analyze`, `rg` 탐색
  - 진행 상태: 사용자 로컬 환경 기준으로 프로젝트에 설치된 GitNexus CLI는 `.git`이 있는 루트(`C:\Dev\health-center-smart-reservation`)에서 `gitnexus status`, `gitnexus analyze`, `gitnexus list`를 직접 실행한다. 여러 저장소가 인덱싱되어 repo 지정이 필요하면 `-r health-center-smart-reservation`을 붙인다. GitNexus가 실패하면 변경 범위 확인은 `git status`, `git diff --stat`, `git diff --check`, `rg`, Maven/Next build로 보완한다.
...
- [x] 샘플 코드 정리 1차
  ...
  - 진행 상태: 레거시 로그인/SNS/관리자 샘플 API, Service, DAO, 테스트, SNS 설정 제거와 Maven compile/test-compile 확인 완료. Swagger/API/프론트 런타임 확인은 사용자 확인 완료. `JwtAuthenticationFilter`의 레거시 `LoginVO` fallback과 `EgovJwtTokenUtil`/`LoginVO` 기반 인증 보조 유틸을 정리했다. 이번 요청 기준상 Maven 빌드/테스트와 Swagger 런타임 확인은 사용자 직접 수행
  - 참고 문서: `docs/10_backend_transition/03_샘플_코드_정리_범위_및_안전_절차.md`, `docs/11_implementation_log/16_eGovFrame_레거시_인증_샘플_비노출_기록.md`, `docs/11_implementation_log/19_eGovFrame_레거시_인증_샘플_제거_기록.md`, `docs/11_implementation_log/94_Jwt_Filter_Legacy_LoginVO_Fallback_정리_기록.md`, `docs/11_implementation_log/96_eGov_JWT_LoginVO_Auth_Utils_Cleanup_기록.md`
...
- [x] Visit Context 구현
  ...
  - 진행 상태: `POST /api/visits/check-in`, `POST /api/visits/walk-in` 구현 및 Maven compile/test-compile 완료. 2026-05-22 직원 접수 UX 개선을 위해 `GET /api/reservations/staff/search`와 예약 검색 후 접수 화면을 추가했다. Swagger/API/프론트 런타임 확인은 사용자 직접 확인 필요
...
- [x] Queue Context 구현
  ...
  - 진행 상태: `GET /api/queues`, `POST /api/queues/{id}/call`, `POST /api/queues/{id}/start`, `POST /api/queues/{id}/complete`, `POST /api/queues/{id}/hold`, `POST /api/queues/{id}/no-show`, `POST /api/queues/{id}/cancel` 구현 및 Maven compile/test-compile 완료. 대기번호 발급 동시성 보강을 위해 채번 테이블, 원자적 upsert 발급 SQL, 당일 대기번호 유니크 인덱스 추가. 2026-05-22 관리자용 `POST /api/queues/admin/close-pending` 미처리 대기표 NO_SHOW 마감 API를 추가하고, 마감 전 확인 화면을 위해 `GET /api/queues`에 `date` 필터를 보강했다. Swagger/API 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/02_domain/02_업무_흐름도.md`
...
- [x] 공통 인증 보조 화면 구현
  ...
  - 진행 상태: `/register`, `/reset-password`, `/find-id` 보조 페이지와 `/login/social/callback` 소셜 로그인 콜백 화면, `/login/social/complete` 추가 정보 입력 화면 추가 완료. 기존 `/social-login` mock 페이지는 제거하고 로그인 화면에 카카오/네이버/구글 버튼을 직접 배치했다. 자동 로그인/아이디 기억 체크박스, 자동 로그인 선택 시 localStorage 토큰 저장, 초기 인증 확인 중 로딩 화면 표시 반영. TypeScript/Next build 통과, 브라우저 확인은 사용자 직접 확인 필요. 2026-05-27 기준 아이디 찾기와 비밀번호 찾기는 화면 mock 상태이며, 실제 API/DB 연동은 후속 고도화 후보로 분리했다.
...
- [x] 직원 접수/체크인 화면 구현
  ...
  - 진행 상태: 예약자 체크인과 현장 접수를 실제 API로 연결하고 TypeScript/Next build 확인 완료. 2026-05-18 접수 저장 시각은 backend/DataSource/DB 세션 시간대 `Asia/Seoul` 기준으로 보정했다. 2026-05-22 예약번호 단독 입력 UX를 보완하기 위해 직원용 예약 검색과 결과 선택 접수 흐름을 추가했다. 브라우저/API 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/05_frontend/02_UX_API_계약_우선순위.md`

- [x] 대기열 관리 화면 구현
  - 완료 기준: 직원이 대기자 목록을 보고 호출, 시작, 완료 처리할 수 있다.
  - 검증 방법: 상태 전이 버튼 동작 확인
  - 진행 상태: 대기열 조회와 호출/시작/완료/보류/미응답/취소 상태 변경을 실제 API로 연결하고 TypeScript/Next build 확인 완료. 2026-05-22 관리자 대기 마감 관리 화면을 추가해 마감 대상 날짜별 미처리 대기표를 확인하고 `POST /api/queues/admin/close-pending`을 실행할 수 있게 했다. 브라우저/API 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/02_domain/02_업무_흐름도.md`
...
| 보류 정리 진행 중 | `EgovJwtTokenUtil`과 `LoginVO` 인증 보조 유틸 제거 | 보건소 Auth/Member 인증 흐름이 `MemberPrincipal` 기준으로 전환되어 레거시 `LoginVO` 인증 보조 묶음을 줄이기 위함 | 코드 정리와 정적 확인 완료. 이번 요청 기준상 Maven 빌드/테스트와 Swagger 런타임 확인은 사용자 직접 수행 | `docs/11_implementation_log/96_eGov_JWT_LoginVO_Auth_Utils_Cleanup_기록.md` |
...
| MVP 이후 고도화 완료 | 공통 예외/오류 응답 일관화 | Controller별 반복 `try/catch`와 문자열 기반 오류 응답 생성을 줄이고 실패 응답에 `traceId`를 포함하기 위함 | `ErrorCode`, `BusinessException`, `GlobalExceptionHandler`, 인증 principal helper 추가, Controller 반복 오류 응답 제거, Maven compile/test-compile과 Next build 확인 완료. 2026-05-20 사용자 Docker 실행 환경에서 Swagger 대표 오류 응답 확인 완료 | `docs/11_implementation_log/82_Common_Error_Response_Refactor_기록.md`, `docs/11_implementation_log/98_DC006_Common_Error_Response_Status_정리_기록.md` |
| MVP 이후 고도화 완료 | Spring Security filter 401/403 공통 오류 응답 정리 | 인증/인가 실패가 Controller에 도달하지 않는 경우에도 신규 API 오류 응답 계약을 유지하기 위함 | `JwtAuthenticationEntryPoint`의 레거시 `ResultVO` 응답을 `ApiResponse.failure(AUTH_REQUIRED)`로 교체하고, `JwtAccessDeniedHandler`를 추가해 권한 부족 403을 `ApiResponse.failure(FORBIDDEN)`로 응답하게 했다. Maven compile/test-compile 확인 완료. Swagger 런타임 확인은 사용자 직접 수행 필요 | `docs/11_implementation_log/100_Security_Filter_Error_Response_구현_기록.md` |
| MVP 이후 고도화 완료 | 보건소 도메인 비즈니스 예외 명시화 | 메시지 기반 `ErrorCode.fromMessage()` 의존을 줄이고 주요 API 오류 코드를 안정화하기 위함 | 예약/방문/대기/오피스/대시보드/인증/소셜 로그인 계층의 `IllegalArgumentException` throw 사용처를 `BusinessException(ErrorCode.X)`로 교체했다. 보건소 도메인 하위 잔여 throw 사용처는 없고, `GlobalExceptionHandler` fallback만 유지한다. Maven compile/test-compile 확인 완료. Swagger 런타임 확인은 사용자 직접 수행 필요 | `docs/11_implementation_log/104_Reservation_Business_Exception_구현_기록.md` |
| MVP 이후 고도화 완료 | 비즈니스 예외 정책 대표 케이스 테스트 | 예외 코드 명시화 이후 정책 계층 오류 코드가 회귀하지 않도록 최소 단위 테스트를 추가하기 위함 | 예약 취소, 방문 체크인, 대기표 상태 전이 정책의 대표 Bad case와 허용 케이스를 순수 단위 테스트로 추가했다. 대상 테스트, 전체 `mvn.cmd -q test`, Maven compile/test-compile, `git diff --check` 통과. DB/동시성/API 테스트는 후속 후보로 유지 | `docs/11_implementation_log/106_Business_Error_Policy_Test_구현_기록.md` |
| 2026-05-22 | 프론트엔드 메타데이터 및 아이콘 설정 | v0 생성 흔적 제거, 프로젝트 전용 아이콘 적용, 문구 자연어화, OG 메타태그와 manifest.json 추가 | `generator: 'v0.app'` 제거, description 자연어 수정, favicon/Apple Touch 아이콘 교체, openGraph 태그 추가, manifest.json 생성, `NEXT_PUBLIC_APP_URL` env 변수 추가, `npm run build` 통과 | `docs/11_implementation_log/116_Frontend_Metadata_Icon_구현_기록.md` |
| 2026-05-22 | 프론트엔드 카카오톡 OG 이미지 표시 오류 수정 | 카카오톡 공유 시 이미지 영역이 비는 문제를 줄이기 위해 OG 이미지 URL과 Docker 빌드 환경변수 전달 기준 보정 | `og:image`를 영문 파일명 `/og-image.png`로 변경, Apple Touch 이미지 경로 영문화, frontend Docker build args/runtime env에 `NEXT_PUBLIC_APP_URL` 추가, README와 PR 문서 갱신, `npm.cmd run build` 통과 | `docs/11_implementation_log/118_Frontend_Kakao_OG_Image_Fix_구현_기록.md`, `docs/11_implementation_log/119_Frontend_Kakao_OG_Image_Fix_PR_작성안.md` |
| 2026-05-27 | 구현 로그 기반 후속작업 통합 색인 문서화와 로그인 안내 정리 | 최신 구현 기록과 PR 작성안에 흩어진 미완 기능, 정책 결정, 운영 고도화 후보를 다음 작업에서 고르기 쉽게 모으고, 로그인 화면의 테스트 비밀번호 노출 문구를 제거하기 위함 | `docs/14_deferred_cleanup/02_후속작업_통합_정리.md` 작성, 배포/k6 확인済 항목의 우선순위 조정, 로그인 페이지 비밀번호 안내 제거, 전체 체크리스트와 README 반영. Maven/Swagger/브라우저 검증은 미수행 | `docs/11_implementation_log/141_Deferred_Followup_Index_문서화_기록.md`, `docs/11_implementation_log/142_Deferred_Followup_Index_PR_작성안.md` |
...
- [ ] 잔여 eGovFrame 샘플/보류 항목 재점검
  ...
  - [x] `EgovJwtTokenUtil` 유지/삭제 여부 결정
  - [x] `LoginVO`와 eGovFrame 사용자 인증 유틸 유지/정리 범위 결정
- [x] 공통 예외/오류 응답 일관화
- [x] Spring Security filter 단계 401/403 공통 오류 응답 정리
- [x] 예약 객체 권한 정책 1차 보강
- [x] 보건소 도메인 비즈니스 예외 명시화
- [x] 비즈니스 예외 정책 대표 케이스 테스트
- [x] 관리자 마감 알림 UX 1차 구현
- [x] 예약 체크인 QR/바코드 표시
- [x] Actuator/Micrometer 운영성 기반 추가
- [x] Prometheus/Grafana/Loki Compose 운영 상태 확인 도구 추가
- [x] k6 운영성 고도화
  - 진행 상태: 예방접종 현장 접수와 대기열 조회, 일부 대기표 호출/시작/완료 흐름을 대상으로 한 k6 시나리오와 Docker/로컬 실행 문서를 추가했고, 사용자가 배포 환경에서 k6 테스트를 수행했다. 추가 증적 문서화는 선택 보강으로 둔다.
- [x] 구현 로그 기반 후속작업 통합 색인 문서화
- [ ] 아이디 찾기 및 비밀번호 찾기 실제 API 연동
  - 진행 상태: 2026-05-27 `feat/account-recovery-api`에서 백엔드 Account Recovery API 1차 구현을 진행했다. `POST /api/auth/find-id`, `POST /api/auth/password-reset/request`, `POST /api/auth/password-reset/confirm`, `password_reset_tokens` 테이블, MyBatis query, 공개 보안 경로, API 명세를 추가했다. `/find-id`, `/reset-password` 프론트 화면은 아직 mock이며 실제 API 연동은 후속 작업이다.
  - 참고 문서: `docs/11_implementation_log/143_Account_Recovery_Implementation_Review_기록.md`, `docs/11_implementation_log/144_Account_Recovery_API_구현_기록.md`, `docs/11_implementation_log/145_Account_Recovery_API_PR_작성안.md`, `docs/14_deferred_cleanup/01_보류_정리_목록.md`, `docs/14_deferred_cleanup/02_후속작업_통합_정리.md`

위 항목은 MVP 완료를 막는 잔여 작업이 아니라 후속 고도화 후보로 관리한다.

### 12.6 다음 브랜치 후보 점검

2026-05-23 현재 다음 브랜치 후보 추천을 위해 아래 문서를 다시 확인했다.

- [x] `docs/README.md` 확인
- [x] `docs/09_agent/05_문서기반_자동진행_운영가이드.md` 확인
- [x] `docs/13_schedule/01_남은_일정_단계별_진행_계획.md` 확인
- [x] `docs/13_schedule/02_전체_작업_체크리스트.md` 확인
- [x] `docs/14_deferred_cleanup/01_보류_정리_목록.md` 확인
- [x] 최근 운영 상태 확인 도구 구현 기록과 PR 문서 확인
- [x] 현재 브랜치와 작업 트리 상태 확인

확인 결과 현재 작업 트리는 깨끗하고, `dev...origin/dev` 상태다.

2026-05-24 현재 다음 작업 후보 추천을 위해 같은 기준 문서와 k6 최신 실행 기록을 다시 확인했다.
...
- [x] `docs/11_implementation_log/133_K6_Vaccination_Queue_Load_Scenarios_구현_기록.md` 확인
- [x] `docs/11_implementation_log/134_K6_Vaccination_Queue_Load_Scenarios_PR_작성안.md` 확인
- [x] 현재 브랜치와 작업 트리 상태 확인

확인 결과 현재 브랜치는 `dev...origin/dev` 상태이며, `git status --short --branch` 기준 작업 트리 변경은 없었다.
최신 k6 기록 기준으로 VM 도메인 대상 VUS 20 부하에서 기능 실패와 HTTP 실패는 없었지만 `GET /api/queues` p95가 2.31초로 조회 기준 1초를 초과했다. 따라서 다음 구현 후보는 배포 후 상태 확인 가이드보다 대기열 조회 성능 개선을 우선 검토한다.

현재 MVP 기능과 배포 기본선은 완료 상태로 보고, 다음 브랜치는 아래 후보 중 하나를 작게 진행한다.

2026-05-27 아이디 찾기/비밀번호 찾기 구현 방식 검토 결과, 다음 인증 UX 후보를 추가한다.

- [ ] 인증 UX 고도화: 아이디 찾기 및 비밀번호 찾기 실제 API 연동
  - 추천 브랜치: `feat/account-recovery-api`
  - 진행 상태: 백엔드 API와 `password_reset_tokens` 테이블, MyBatis query는 1차 구현했다. 프론트 API client와 `/find-id`, `/reset-password` 실제 연동, Swagger/브라우저 검증은 남아 있다.
  - 참고 문서: `docs/11_implementation_log/143_Account_Recovery_Implementation_Review_기록.md`, `docs/11_implementation_log/144_Account_Recovery_API_구현_기록.md`, `docs/11_implementation_log/145_Account_Recovery_API_PR_작성안.md`, `docs/14_deferred_cleanup/01_보류_정리_목록.md`

- [x] 1순위: 관리자 마감 알림 UX
  - 추천 브랜치: `feat/admin-queue-closing-alert`
  - 진행 상태: 17:30 이후 오늘 미처리 대기표가 있으면 관리자 화면 상단 배너와 토스트로 알리고, `/admin/queue-closing` 이동과 브라우저 단위 숨김을 제공한다. 17:30 전에는 `setTimeout`으로 대기하고 백그라운드 탭에서는 주기 조회를 멈춘다. `npm.cmd exec -- tsc --noEmit`, `npm.cmd run build`, `git diff --check` 통과. 브라우저/API 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/120_Admin_Queue_Closing_Alert_UX_구현_기록.md`, `docs/11_implementation_log/121_Admin_Queue_Closing_Alert_UX_PR_작성안.md`
- [x] 2순위: 예약 체크인 QR/바코드 생성 및 표시
  - 추천 브랜치: `feat/reservation-checkin-qr-code`
  - 진행 상태: 예약번호 기반 QR/Code128 바코드 공통 컴포넌트를 추가하고, 예약 완료/내 예약/직원 체크인 검색 결과에 표시했다. 직원 직접 입력 안내도 스캐너 입력 흐름에 맞게 보강했다. `npm.cmd exec -- tsc --noEmit`, `npm.cmd run build`, `git diff --check` 통과. 브라우저 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/122_Reservation_Checkin_QR_Barcode_구현_기록.md`, `docs/11_implementation_log/123_Reservation_Checkin_QR_Barcode_PR_작성안.md`
- [x] 3순위: Actuator/Micrometer 운영성 고도화
  - 추천 브랜치: `feat/backend-observability-actuator`
  - 진행 상태: `spring-boot-starter-actuator` 기존 적용 상태에 `micrometer-registry-prometheus`를 추가하고, `/actuator/health`, `/actuator/info`, `/actuator/metrics`, `/actuator/prometheus` 노출 설정과 보안 정책을 정리했다. health/info는 공개하고 metrics/prometheus 및 기타 actuator endpoint는 ADMIN 보호로 둔다. `mvn.cmd -q -DskipTests compile`, `mvn.cmd -q test-compile`, `git diff --check` 통과. 서버 기동, Docker 실행, API 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/124_Backend_Observability_Actuator_구현_기록.md`, `docs/11_implementation_log/125_Backend_Observability_Actuator_PR_작성안.md`
- [x] 4순위: Prometheus/Grafana/Loki Compose 운영 상태 확인 도구
  - 추천 브랜치: `chore/observability-compose-stack`
  - 진행 상태: Docker Compose `observability` profile에 Prometheus, Grafana, Loki, Promtail을 추가하고, backend `/actuator/prometheus` scrape 설정, Grafana datasource/dashboard provisioning, Promtail Docker log 수집 설정을 구성했다. `OBSERVABILITY_PROMETHEUS_SCRAPE_ENABLED=true`일 때 Prometheus scrape를 허용하고 기본값은 보호 상태로 유지한다. `mvn.cmd -q -DskipTests compile`, `mvn.cmd -q test-compile`, `git diff --check` 통과. Docker 실행과 Prometheus/Grafana/Loki 런타임 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/126_Observability_Compose_Stack_구현_기록.md`, `docs/11_implementation_log/127_Observability_Compose_Stack_PR_작성안.md`
- [x] 4-1순위: Jenkins 운영 상태 확인 도구 profile 배포 보정
  - 추천 브랜치: `fix/jenkins-observability-profile-deploy`
  - 진행 상태: Jenkins Pipeline이 `docker compose --env-file .env`만 실행해 `profiles: observability` 서비스가 배포 대상에서 빠지는 원인을 확인했다. Jenkinsfile의 `config/build/up/ps` 단계에 `--profile observability`를 적용해 VM Jenkins 배포 후 Prometheus/Grafana/Loki/Promtail 컨테이너가 함께 생성되도록 보정했다. `git diff --check` 통과. Jenkins Pipeline, Docker 컨테이너, 브라우저 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/128_Jenkins_Observability_Profile_Deploy_구현_기록.md`, `docs/11_implementation_log/129_Jenkins_Observability_Profile_Deploy_PR_작성안.md`
- [x] 4-2순위: Jenkins 운영 상태 확인 도구 설정 파일 bind mount 오류 구조 해결
  - 추천 브랜치: `fix/observability-config-image-mounts`
  - 진행 상태: Jenkins 컨테이너 내부 workspace 파일을 Docker host bind mount source로 사용하면서 Prometheus 설정 파일 마운트가 실패한 원인을 확인했다. Prometheus/Grafana/Loki/Promtail 설정 파일을 각 custom image에 포함하고, Compose의 설정 파일 bind mount를 제거했다. Promtail의 Docker log 수집용 host mount만 유지한다. `git diff --check` 통과. Jenkins Pipeline, Docker build/deploy, 브라우저 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/130_Observability_Config_Image_Mounts_구현_기록.md`, `docs/11_implementation_log/131_Observability_Config_Image_Mounts_PR_작성안.md`
- [x] 5순위: k6 부하 테스트 시나리오
  - 추천 브랜치: `test/k6-queue-load-scenarios`
  - 진행 상태: 배포 VM 주소를 `BASE_URL` 환경변수로 받는 예방접종 현장 접수/대기 처리 k6 시나리오를 추가했다. Docker 실행을 기본으로 문서화하고, 로컬 k6 실행 방식도 함께 정리했다. 기본값은 `VUS=5`, `DURATION=1m`, `QUEUE_ACTION_RATE=0.3`이며 실패율, 조회 p95, 쓰기 p95 threshold를 환경변수로 조절할 수 있다. 실제 VM 부하 테스트, Grafana/Loki 확인은 사용자 직접 확인 필요
  - 참고 문서: `performance/k6/README.md`, `docs/11_implementation_log/133_K6_Vaccination_Queue_Load_Scenarios_구현_기록.md`, `docs/11_implementation_log/134_K6_Vaccination_Queue_Load_Scenarios_PR_작성안.md`
- [x] 6순위: 대기열 조회 성능 개선
  - 추천 브랜치: `perf/queue-list-query-optimization`
  - 진행 상태: k6 VM VUS 20 실행에서 `GET /api/queues` p95가 2.31초로 조회 기준 1초를 초과한 병목 후보를 1차 개선했다. `GET /api/queues`에 `limit` 파라미터를 추가하고 기본 200건/최대 500건으로 제한했다. SQL은 `issued_at::date` 조건을 하루 범위 조건으로 바꾸고 `LIMIT`을 적용했으며, `queue_tickets` 조회용 복합 인덱스를 추가했다. k6 `queue_list` 요청에는 `limit=100`을 명시했다. `mvn.cmd -q -DskipTests compile`, `mvn.cmd -q test-compile`, `npm.cmd run build`, `git diff --check` 통과. Swagger/API/브라우저/k6 재측정은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/135_Queue_List_Query_Optimization_구현_기록.md`, `docs/11_implementation_log/136_Queue_List_Query_Optimization_PR_작성안.md`
- [x] 7순위: 배포 후 상태 확인과 문제 해결 가이드 문서화
  - 추천 브랜치: `docs/observability-troubleshooting-runbook`
  - 진행 상태: Jenkins `observability` profile 적용 여부, 설정 파일 bind mount 오류 재발 확인, Prometheus target, Grafana datasource/dashboard, Loki/Promtail 로그 수집 확인 절차를 초보자가 따라가기 쉬운 배포 후 상태 확인 가이드로 정리했다. 코드 변경 없이 `docs/08_deploy/07_배포후_상태확인_문제해결_가이드.md`를 추가하고 docs README, 구현 기록, PR 문서 초안을 갱신했다. `git diff --check` 통과. Jenkins Pipeline, Docker Compose, Prometheus/Grafana/Loki 브라우저 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/08_deploy/07_배포후_상태확인_문제해결_가이드.md`, `docs/11_implementation_log/137_Deploy_Status_Check_Guide_작성_기록.md`, `docs/11_implementation_log/138_Deploy_Status_Check_Guide_PR_작성안.md`
- [x] 8순위: 미처리 대기표 자동 NO_SHOW 배치
  - 추천 브랜치: `feat/auto-close-pending-queue-batch`
  - 진행 상태: Spring Scheduler 기반 자동 마감 배치를 추가했다. 기본 설정은 `QUEUE_AUTO_CLOSE_ENABLED=true`, `QUEUE_AUTO_CLOSE_CRON=0 10 18 * * *`, `QUEUE_AUTO_CLOSE_RETENTION_DAYS=2`이며, 실행일 기준 2일 전까지 발급된 `WAITING`, `CALLED`, `HOLD`, `IN_PROGRESS` 대기표와 연결 Visit/Reservation을 기존 수동 마감과 같은 규칙으로 `NO_SHOW` 처리한다. 재실행 시 이미 `NO_SHOW` 처리된 건은 제외되며 `event=queue.pending_auto_closed` 로그를 남긴다. `mvn.cmd -q -DskipTests compile`, `mvn.cmd -q test-compile`, `mvn.cmd -q "-Dtest=QueueCommandServiceTest" test`, `git diff --check` 통과. 서버 기동과 실제 스케줄 실행 로그 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/11_implementation_log/139_Auto_Close_Pending_Queue_Batch_구현_기록.md`, `docs/11_implementation_log/140_Auto_Close_Pending_Queue_Batch_PR_작성안.md`
