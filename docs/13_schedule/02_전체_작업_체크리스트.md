...
- [ ] 아이디 찾기 및 비밀번호 찾기 실제 API 연동
  - 진행 상태: 2026-05-27 현재 `/find-id`, `/reset-password` 프론트 화면은 mock이고 백엔드 Account Recovery API와 재설정 토큰 저장 구조는 없다. 권장 방식은 아이디 찾기 이메일 마스킹 반환과 서버 저장 토큰 기반 2단계 비밀번호 재설정이다.
  - 참고 문서: `docs/11_implementation_log/143_Account_Recovery_Implementation_Review_기록.md`, `docs/14_deferred_cleanup/01_보류_정리_목록.md`, `docs/14_deferred_cleanup/02_후속작업_통합_정리.md`
...
- [ ] 인증 UX 고도화: 아이디 찾기 및 비밀번호 찾기 실제 API 연동
  ...
  - 진행 상태: 현재 화면은 mock이며 백엔드 API, `password_reset_tokens` 후보 테이블, MyBatis query, 프론트 API client 연동이 필요하다. 구현 방식 후보는 `docs/11_implementation_log/143_Account_Recovery_Implementation_Review_기록.md`에 정리했다.
  - 참고 문서: `docs/11_implementation_log/143_Account_Recovery_Implementation_Review_기록.md`, `docs/14_deferred_cleanup/01_보류_정리_목록.md`
