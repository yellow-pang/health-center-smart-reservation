...
- [ ] 환경변수 정리
  ...
  - 진행 상태: 루트 `.env.example` 작성, `.gitignore`의 `.env` 제외 확인, backend DB/JWT/CORS placeholder 적용, Maven compile/test-compile 완료. Frontend Dockerfile/전체 Compose/Jenkinsfile과 실제 Docker 런타임 검증은 후속 브랜치에서 진행
  - 참고 문서: `docs/08_deploy/*`
...
| 운영/배포 구현 진행 중 | 백엔드 환경변수와 Dockerfile 정리 | Docker Compose/Jenkins 배포 전에 backend DB/JWT/CORS 값을 `.env`에서 주입하고 Maven jar를 컨테이너 이미지로 만들 수 있어야 함 | `.env.example`, backend placeholder, CORS Origin 환경변수화, `backend/Dockerfile`, `backend/.dockerignore` 작성 및 Maven compile/test-compile 완료. Docker build/runtime 검증은 사용자 직접 수행 | `docs/11_implementation_log/74_Backend_Env_Dockerfile_구현_기록.md` |
...
2026-05-16 기준, Ubuntu VM + Jenkins + Docker Compose 배포 계획서, PR 진행용 문서, 루트 `.env.example`, backend 환경변수 placeholder, CORS Origin 환경변수화, `backend/Dockerfile`은 작성했다. 다만 `frontend/Dockerfile`, 전체 `docker-compose.yml`, Jenkinsfile, README 배포 섹션과 Docker/Jenkins 런타임 검증은 아직 완료하지 않았으므로 위 항목은 완료 처리하지 않는다.
