...
- [ ] Docker Compose 실행 정리
  ...
  - 진행 상태: Ubuntu VM + Jenkins + Docker Compose 배포 계획서 작성 완료. 현재 루트 `docker-compose.yml`은 PostgreSQL 단일 서비스만 포함하되 DB명/계정/비밀번호/포트는 `.env` 기반으로 전환했다. backend/frontend 포함 전체 Compose 구현은 후속 브랜치에서 진행
  - 참고 문서: `docs/08_deploy/*`

- [ ] 환경변수 정리
  - 완료 기준: DB 접속 정보, 토큰 secret, 프로파일 설정이 문서화된다.
  - 검증 방법: 민감 정보가 직접 노출되지 않았는지 확인
  - 진행 상태: 루트 `.env.example` 작성, 로컬 `.env` 생성, `.gitignore`의 `.env` 제외 확인, backend DB/JWT/CORS placeholder 적용, PostgreSQL Compose 환경변수화, Maven compile/test-compile 완료. Frontend Dockerfile/전체 Compose/Jenkinsfile과 실제 Docker 런타임 검증은 후속 브랜치에서 진행
  - 참고 문서: `docs/08_deploy/*`
...
2026-05-16 기준, Ubuntu VM + Jenkins + Docker Compose 배포 계획서, PR 진행용 문서, 루트 `.env.example`, 로컬 `.env`, backend 환경변수 placeholder, CORS Origin 환경변수화, PostgreSQL Compose 환경변수화, `backend/Dockerfile`은 작성했다. 다만 `frontend/Dockerfile`, backend/frontend 포함 전체 `docker-compose.yml`, Jenkinsfile, README 배포 섹션과 Docker/Jenkins 런타임 검증은 아직 완료하지 않았으므로 위 항목은 완료 처리하지 않는다.
