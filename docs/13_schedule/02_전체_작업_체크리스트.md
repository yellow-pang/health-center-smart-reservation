...
- [ ] Visit Context 구현
  ...
  - 진행 상태: `POST /api/visits/check-in` 예약자 체크인 구현 및 Maven compile/test-compile 완료. Swagger 체크인 seed 예약번호는 `RSV-SWAGGER-CHECKIN-001`, 런타임 검증은 사용자가 Docker PostgreSQL, VS Code Spring Boot Dashboard, Swagger 대표 예시로 직접 진행, 현장 접수는 후속
...
- [ ] Seed 데이터 정리
  ...
  - 진행 상태: 공통코드, 기본 보건소, 기본 회원, 업무 유형, 창구, 창구 업무 매핑, 예약 상세/취소/체크인 Swagger 테스트용 예약 seed 작성 완료. 런타임 초기화 확인은 사용자 직접 확인 필요
