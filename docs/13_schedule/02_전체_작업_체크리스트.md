...
| 미검증 | 서버 기동, Docker 실행, Swagger `Try it out`, 브라우저 화면 확인 | 남은 작업 리스트로 분리 |

체크 표시가 된 구현 항목도 런타임 검증까지 완료됐다는 뜻은 아니다.
서버/Docker/Swagger/브라우저 확인은 프로젝트 운영 기준에 따라 사용자가 직접 수행해야 하므로 `12. 현재 남은 작업 리스트`에 별도로 남긴다.
...
- [x] 샘플 코드 정리 1차
  ...
  - 진행 상태: 레거시 로그인/SNS/관리자 샘플 API, Service, DAO, 테스트, SNS 설정 제거와 Maven compile/test-compile 확인 완료. Swagger 노출 확인과 잔여 샘플/보류 항목 재점검은 남은 작업으로 분리
...
- [x] 공통 응답/예외 구조 안정화
  ...
  - 진행 상태: `ApiResponse` 공통 응답과 주요 Controller의 성공/실패 응답 사용 확인. 실제 실패 응답 JSON 런타임 확인은 남은 작업
  - 참고 문서: `docs/09_agent/02_에이전트_프롬프트_및_코드작성_지침.md`

- [x] Office Context 구현
  - 완료 기준: 업무 유형 조회 API가 동작한다.
  - 검증 방법: `GET /api/service-types` 호출
  - 진행 상태: 업무 유형 조회, 관리자 업무 유형 전체 조회/생성/수정/비활성화/재활성화, 직원 목록/생성/수정/비활성화, 창구 조회/생성/수정/비활성화/담당자 배정/업무 매핑 구현 및 Maven compile/test-compile 완료. 런타임 API 호출은 남은 작업
...
- [x] Auth/Member Context 구현
  ...
  - 진행 상태: 로그인, 현재 사용자 조회, 토큰 재발급, 로그아웃, URL 기반 역할별 접근 규칙 구현 및 Maven compile/test-compile 완료. 사용자 직접 런타임 확인과 객체 권한 정책은 남은 작업
...
- [x] Reservation Context 구현
  ...
  - 진행 상태: 예약 슬롯 테이블, Swagger 테스트용 예약 seed, `GET /api/reservation-slots`, `POST /api/admin/reservation-slots`, `PUT /api/admin/reservation-slots/{id}`, `PATCH /api/admin/reservation-slots/{id}/deactivate`, `POST /api/reservations`, `GET /api/reservations/me`, `GET /api/reservations/{id}`, `DELETE /api/reservations/{id}` 구현 및 Maven compile/test-compile 완료. Swagger Happy/Edge/Bad 확인은 남은 작업
...
- [x] Visit Context 구현
  ...
  - 진행 상태: `POST /api/visits/check-in`, `POST /api/visits/walk-in` 구현 및 Maven compile/test-compile 완료. 런타임 검증은 사용자가 Docker PostgreSQL, VS Code Spring Boot Dashboard, Swagger 대표 예시로 직접 진행
...
- [x] Dashboard/Congestion Context 구현
  ...
  - 진행 상태: `GET /api/dashboard/summary`, `GET /api/congestion/current`, `GET /api/dashboard/hourly-visits`, `GET /api/dashboard/service-wait-times`, `GET /api/dashboard/visit-type-ratio`, `GET /api/dashboard/no-show-rate` 구현 및 Maven compile/test-compile 완료. Swagger 통계 응답 확인은 남은 작업
...
- [x] 공통 인증 보조 화면 구현
  ...
  - 진행 상태: `/register`, `/social-login`, `/reset-password`, `/find-id` mock 페이지 추가 및 로그인 화면 링크 보강 완료. TypeScript/Next build 통과, 브라우저 확인과 실제 API 연동은 사용자 직접 확인 및 후속 작업 필요
...
- [x] PostgreSQL schema 정리
  ...
  - 진행 상태: 공통코드, 회원, 업무 유형, 예약 슬롯/예약, 방문, 대기표/채번, 창구/창구 업무 매핑, Refresh Token 테이블 SQL 작성 및 Maven compile/test-compile 완료. Docker PostgreSQL 실제 적용 확인은 남은 작업
...
- [x] MyBatis Mapper 정합성 확인
  ...
  - 검증 방법: 빌드와 API 호출 확인
  - 진행 상태: `src/main/resources/egovframework/mapper/healthcenter` 하위 PostgreSQL Mapper XML과 Java Mapper 기반 Maven compile/test-compile 완료. API 런타임 호출 확인은 남은 작업
...
- [ ] API 수동 검증
  ...
  - 진행 기준: 터미널 PowerShell/curl 예시는 보조로만 사용하고, 브랜치 구현 기록에는 대표 예시 1개만 둔다. 새 API는 Swagger 대표 예시가 바로 실행되도록 seed/mock 데이터 필요 여부를 확인한다.
  - 참고 문서: 브랜치별 구현 기록
...
| 문서 현황판 정리 중 | 전체 체크리스트 현재 구현 상태 재점검 | 이미 구현/빌드 확인된 항목과 실제 남은 런타임/운영/테스트 작업을 구분하기 위함 | 문서 갱신 완료, Git diff/Maven/Next build 확인 완료 | `docs/11_implementation_log/70_Checklist_Current_State_Update_기록.md` |
...
- [ ] Docker PostgreSQL 실행 후 `schema.sql`, `data.sql` 초기화 확인
- [ ] 백엔드 서버를 사용자가 직접 실행하고 Swagger 접속 확인
- [ ] Auth 대표 흐름 Swagger 확인: `POST /api/auth/login`
- [ ] Reservation 대표 흐름 Swagger 확인: 예약 슬롯 조회, 예약 신청, 내 예약 조회, 예약 취소
- [ ] Visit/Queue 대표 흐름 Swagger 확인: 예약자 체크인 또는 현장 접수 후 대기 호출, 시작, 완료
- [ ] Dashboard/Congestion 대표 흐름 Swagger 확인: 대시보드 요약과 현재 혼잡도 조회
- [ ] 관리자 기준정보 대표 흐름 Swagger 확인: 업무 유형, 예약 슬롯, 직원, 창구 생성/수정/비활성화
...
- [ ] 시민 내 예약/취소 화면 확인
- [ ] 직원 체크인/현장 접수 화면 확인
- [ ] 직원 대기열 호출/시작/완료/보류/미응답/취소 화면 확인
- [ ] 관리자 대시보드 차트와 지표 화면 확인
- [ ] 관리자 업무 유형/예약 슬롯/직원/창구 기준정보 화면 확인
- [ ] 권한 없음 화면과 API 403 이동 처리 확인

### 12.3 테스트 보강

- [ ] 예약 정원 초과 방지 테스트
- [ ] 동일 사용자 동일 시간대 중복 예약 방지 테스트
- [ ] 예약 취소 가능 시간 정책 테스트
- [ ] 예약자 체크인 중복 방지 테스트
- [ ] 현장 접수 대기번호 발급 테스트
- [ ] Queue 상태 전이 정책 테스트
- [ ] STAFF/ADMIN/CITIZEN 권한별 접근 테스트
- [ ] Dashboard SQL 집계 정확성 테스트
