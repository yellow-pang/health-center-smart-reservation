...
- [ ] Queue Context 구현
  ...
  - 진행 상태: `GET /api/queues`, `POST /api/queues/{id}/call`, `POST /api/queues/{id}/start`, `POST /api/queues/{id}/complete`, `POST /api/queues/{id}/hold` 구현 및 Maven compile/test-compile 완료. 런타임 검증은 사용자가 Docker PostgreSQL, VS Code Spring Boot Dashboard, Swagger 대표 순서로 직접 진행, NO_SHOW/취소/동시성 보강은 후속
  - 참고 문서: `docs/02_domain/02_업무_흐름도.md`
...
- [ ] Seed 데이터 정리
  ...
  - 진행 상태: 공통코드, 기본 보건소, 기본 회원, 업무 유형, 창구, 창구 업무 매핑, 예약 상세/취소/체크인 Swagger 테스트용 예약 seed, 대기열 라이프사이클 Swagger 테스트용 대기표 seed 작성 완료. 런타임 초기화 확인은 사용자 직접 확인 필요
