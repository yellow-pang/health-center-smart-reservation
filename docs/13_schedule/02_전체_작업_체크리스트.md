...
| MVP 이후 고도화 진행 중 | 예약 도메인 비즈니스 예외 명시화 | 메시지 기반 `ErrorCode.fromMessage()` 의존을 줄이고 예약 API 오류 코드를 안정화하기 위함 | 예약/예약 슬롯 서비스와 예약 취소 정책의 `IllegalArgumentException`을 `BusinessException(ErrorCode.X)`로 교체했다. `RESERVATION_FORBIDDEN`, `RESERVATION_SLOT_DUPLICATED` 오류 코드를 추가했고 Maven compile/test-compile 확인 완료. Swagger 런타임 확인은 사용자 직접 수행 필요 | `docs/11_implementation_log/104_Reservation_Business_Exception_구현_기록.md` |
