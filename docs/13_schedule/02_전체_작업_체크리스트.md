...
- [ ] Visit Context 구현
  ...
  - 진행 상태: `POST /api/visits/check-in` 예약자 체크인 구현 및 Maven compile/test-compile 완료. 런타임 검증은 사용자가 Docker PostgreSQL, VS Code Spring Boot Dashboard, Swagger 대표 예시로 직접 진행, 현장 접수는 후속
...
- [ ] API 수동 검증
  ...
  - 진행 기준: 터미널 PowerShell/curl 예시는 보조로만 사용하고, 브랜치 구현 기록에는 대표 예시 1개만 둔다.
  - 참고 문서: 브랜치별 구현 기록
