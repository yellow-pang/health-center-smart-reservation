...
2026-05-17 현재 상태 재확인 결과, 작업 트리는 점검 전 기준으로 깨끗했고 루트 `docker-compose.yml`, `backend/Dockerfile`, `frontend/Dockerfile`, `infra/jenkins/Dockerfile`, `infra/jenkins/docker-compose.jenkins.yml`, `Jenkinsfile`, `.env.example`이 존재함을 확인했다. `git diff --check`, `mvn.cmd -q -DskipTests compile`, `mvn.cmd -q test-compile`, `npm.cmd run build`는 통과했다. 단, GitNexus는 stale 상태이며 `npm.cmd exec -- gitnexus analyze`는 "Not inside a git repository", `npm.cmd exec -- gitnexus analyze .`는 비정상 종료되어 인덱스 갱신을 완료하지 못했다.
...
- [x] GitNexus와 `rg` 사용 기준 확인
  ...
  - 진행 상태: GitNexus CLI 의존성 복구 후 `status` 확인, stale index 확인, 이전 `analyze .`는 `.gitnexus/lbug.shadow` 접근 거부로 실패하여 `rg`와 Maven 검증으로 보완. 2026-05-17 재확인에서도 `status`는 stale이며, `analyze`는 저장소 인식 실패, `analyze .`는 비정상 종료되어 갱신 미완료
