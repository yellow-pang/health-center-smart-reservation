...
- [ ] Queue Context 구현
  ...
  - 진행 상태: `GET /api/queues`, `POST /api/queues/{id}/call`, `POST /api/queues/{id}/start`, `POST /api/queues/{id}/complete` 구현 및 Maven compile/test-compile 완료. 런타임 검증은 사용자가 Docker PostgreSQL, VS Code Spring Boot Dashboard, Swagger 대표 순서로 직접 진행, 보류/취소/동시성 보강은 후속
  - 참고 문서: `docs/02_domain/02_업무_흐름도.md`
