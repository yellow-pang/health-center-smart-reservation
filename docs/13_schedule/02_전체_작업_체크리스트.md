...
- [x] Prometheus/Grafana/Loki Compose 관측성 스택 추가
- [x] k6 운영성 고도화
...
최신 k6 기록 기준으로 VM 도메인 대상 VUS 20 부하에서 기능 실패와 HTTP 실패는 없었지만 `GET /api/queues` p95가 2.31초로 조회 기준 1초를 초과했다. 따라서 다음 구현 후보는 문서 Runbook보다 대기열 조회 성능 개선을 우선 검토한다.
...
- [x] 4순위: Prometheus/Grafana/Loki Compose 관측성 스택
...
- [x] 4-1순위: Jenkins 관측성 profile 배포 보정
...
- [x] 4-2순위: Jenkins 관측성 설정 파일 bind mount 오류 구조 해결
...
- [ ] 7순위: 관측성 트러블슈팅 Runbook 문서화
  - 추천 브랜치: `docs/observability-troubleshooting-runbook`
  - 근거: Jenkins 관측성 profile, bind mount, Prometheus/Grafana/Loki 확인 절차와 실패 대응을 한 문서로 묶으면 다음 배포 검증과 포트폴리오 설명에 바로 활용할 수 있음
