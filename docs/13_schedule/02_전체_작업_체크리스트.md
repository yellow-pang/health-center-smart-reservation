...
- [x] 1순위: 관리자 마감 알림 UX
  ...
  - 진행 상태: 17:30 이후 오늘 미처리 대기표가 있으면 관리자 화면 상단 배너와 토스트로 알리고, `/admin/queue-closing` 이동과 세션 단위 숨김을 제공한다. `npm.cmd exec -- tsc --noEmit`, `npm.cmd run build`, `git diff --check` 통과. 브라우저/API 런타임 확인은 사용자 직접 확인 필요
