...
- [ ] Reservation Context 구현
  ...
  - 진행 상태: 예약 슬롯 테이블, seed, `GET /api/reservation-slots`, `POST /api/admin/reservation-slots`, `POST /api/reservations`, `GET /api/reservations/me`, `GET /api/reservations/{id}` 구현 및 Maven compile/test-compile 완료. 예약 취소는 미구현
