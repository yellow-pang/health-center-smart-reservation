...
- [x] 공통 인증 보조 화면 구현
  ...
  - 진행 상태: `/register`, `/social-login`, `/reset-password`, `/find-id` mock 페이지 추가 및 로그인 화면 링크 보강 완료. 로그인 화면의 시민/직원/관리자 테스트 로그인 버튼 제거, 자동 로그인/아이디 기억 체크박스 추가, 자동 로그인 선택 시에만 localStorage 토큰 저장 적용. TypeScript/Next build 통과, 브라우저 확인은 사용자 직접 확인 필요
