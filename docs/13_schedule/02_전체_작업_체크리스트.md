...
- [x] 공통 인증 보조 화면 구현
  ...
  - 진행 상태: `/register`, `/social-login`, `/reset-password`, `/find-id` mock 페이지 추가 및 로그인 화면 링크 보강 완료. 로그인 화면의 시민/직원/관리자 테스트 로그인 버튼 제거, 자동 로그인/아이디 기억 체크박스 추가, 자동 로그인 선택 시에만 localStorage 토큰 저장 적용, 초기 인증 확인 중 로딩 화면 표시. TypeScript/Next build 통과, 브라우저 확인은 사용자 직접 확인 필요
  - 참고 문서: `docs/05_frontend/01_화면_설계서_v0_프롬프트.md`, `docs/11_implementation_log/52_Frontend_Auth_Aux_Pages_구현_기록.md`, `docs/11_implementation_log/88_Frontend_Login_Auto_Remember_구현_기록.md`
