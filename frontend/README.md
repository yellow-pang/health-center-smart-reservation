...
기본 테스트 계정 비밀번호는 `password1234`입니다. 로그인 성공 후 access token이 남아 있으면 `/login` 진입 시 현재 사용자 정보를 확인한 뒤 역할별 첫 화면으로 자동 이동합니다. `아이디 기억`을 선택하면 이메일만 브라우저 localStorage에 저장합니다.
